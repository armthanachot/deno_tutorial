...
const findByRange = async (
  ...
  try {
    ...
    const product: Product | any = products.filter((product) =>
...
const update = async (
  ...
  try {
    let product: Product | any = products.filter((product) =>
      product.id == params.id
    )[0];
    const data = await request.body().value;
    product = data;
...
export { create, destroy, findAll, findByID, findByRange, update };
